"""metro_v36.py — RE-NIVELADO (config v3.6 patrulla estática + capa K; física v3.5 intacta).
100 semillas/tipo, artefactos en /data/hrl_m1/m1pp/metro/. Piezas:
...
OUT = pathlib.Path("/data/hrl_m1/m1pp/metro"); OUT.mkdir(parents=True, exist_ok=True)
...
def piece(name):
    if name == "floor":
        res = evaluate(coordinator_factory=_residual(None)); out = "floor_residual_v36.json"
    elif name == "run02":
        ...
        out = "run02_en_v36_estatica.json"
    elif name == "run02o":
        ...
        out = "run09_en_v36_estatica.json"
    elif name == "run09o":
