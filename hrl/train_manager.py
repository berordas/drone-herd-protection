...
Variante pre-registrada (flag): --fixed-k 1000 (RUN-M2: opciones interrumpidas a K fijo).
...
def make_env(seed, opponent, fixed_k):
    def _f():
        return ManagerEnv(kinds=("lobos",), seed=seed, opponent=opponent, fixed_k=fixed_k)
    return _f


def light_eval(model, opponent: str, fixed_k) -> dict:
    ...
    env = ManagerEnv(kinds=("lobos",), seed=12345, opponent=opponent, fixed_k=fixed_k)
    sev, dec, pen = [], [], []
...
def main():
    ...
    class CB(BaseCallback):
        def __init__(self):
            ...
            self.acts = np.zeros(N_OPTIONS); self.events = {}
        ...
        def _on_step(self):
            ...
            if t >= self.next_eval:
                ...
                ev = light_eval(self.model, args.opponent, args.fixed_k)
    ...
    ev = light_eval(model, args.opponent, args.fixed_k)
