...
- **DISUASIÓN del dron (`_apply_deterrence`, infraestructura, gateada por `escort_enabled`):** dentro de `DETER_RADIUS=40` de un dron **ACTIVE** el lobo **ESQUIVA** (repulsión con falloff lineal, suma de todos los drones a tiro) y **FRENA** (rapidez máx × `DETER_SLOWDOWN=0.5`). `DETER_REPULSION=8` (> `wolf_speed`). **PARCIAL A CORTA** (`deter_w`): un lobo PEGADO a su presa (dist ≤ `r_face_safe`) IGNORA el dron y **empuja a través** (la persecución domina), rampa a disuasión completa en 2·`r_face_safe` → el dron **REDUCE/RETRASA** la caza y despeja pines (aparta a los que se ACERCAN de lejos) pero **NO invulnerabiliza** a una presa clavada. **Sin habituación** (flag #6). Combate puro (`escort_enabled=False`) NO disuade → face_check bit a bit. El POSICIONAMIENTO de los drones = COORDINADOR (post-v2); aquí solo el efecto del mundo.
...
3a movimiento de drones ✓ → 3b detectar→confirmar ✓ → **paso 2 guiado al refugio ✓** → **disuasión del dron ✓** → **matanza excedente ✓** → **fix pin + envolvente ✓** → **evitación al huir ✓** → **el más cercano investiga ✓** → 3c corzos → **congelar v2** → **coordinador: posicionar/interponer drones** (baja la SEVERIDAD) + reflejo-reactivo (consume el mensaje) → MARL.
*(Los coordinadores van DESPUÉS de congelar v2: v2 es la referencia que deben batir. Baseline HONESTA con Dummy+guiado+disuasión PASIVA: TASA ~72%, **SEVERIDAD ~2.33 muertes/ep** (el pin la subió desde ~1.55 FALSEADA; el rodeo la bajó desde ~2.73; el reflejo del más cercano la dejó igual, +0.06 ruido; máx. 7). Adversario puro ~6.33. La SEVERIDAD es la métrica principal; la baja el coordinador posicionando drones para despejar pines activamente.)*
