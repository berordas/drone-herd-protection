...
3a movimiento de drones ✓ → 3b detectar→confirmar ✓ → **paso 2 guiado al refugio ✓** → **disuasión del dron ✓** → **matanza excedente ✓** → **fix pin + envolvente ✓** → 3c corzos → **congelar v2** → **coordinador: posicionar/interponer drones** (baja la SEVERIDAD) + reflejo-reactivo (consume el mensaje) → MARL.
*(Los coordinadores van DESPUÉS de congelar v2: v2 es la referencia que deben batir. Baseline HONESTA (arreglado el pin) con Dummy+guiado+disuasión PASIVA: TASA ~78%, **SEVERIDAD ~2.73 muertes/ep** (antes ~1.55 FALSEADA a la baja por pines invulnerables; máx. 8). Adversario puro ~6.33. La SEVERIDAD es la métrica principal; la baja el coordinador posicionando drones para despejar pines activamente.)*
...
## Estado (commits)
`194a3ad` base · `37910b3` terminal · `e663504` disparador por dron · `4d1e708` campo 300 + escala absoluta · `886bd45` dispersión · `a15e2df` mov. drones (3a) · `fd893b8` detectar→confirmar (3b) · `49e0e22` consolidar docs · `144b7bd` paso 2 guiado · `1d44cdc` máx. 1 caza (REVERTIDO) · `56ff75d` huida no-holonómica · `f42456f` 2 fixes huida · `26bce79` disuasión del dron · `bd57a8f` matanza excedente · **fix pin (envolvente + disuasión parcial a corta): adulta clavada matable; baseline honesta sev~2.73 (este commit)**.
