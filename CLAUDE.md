...
3a movimiento de drones ✓ → 3b detectar→confirmar ✓ → **paso 2 guiado al refugio ✓** → **disuasión del dron ✓** → **matanza excedente ✓** → **fix pin + envolvente ✓** → **evitación al huir ✓** → 3c corzos → **congelar v2** → **coordinador: posicionar/interponer drones** (baja la SEVERIDAD) + reflejo-reactivo (consume el mensaje) → MARL.
*(Los coordinadores van DESPUÉS de congelar v2: v2 es la referencia que deben batir. Baseline HONESTA con Dummy+guiado+disuasión PASIVA: TASA ~72%, **SEVERIDAD ~2.27 muertes/ep** (el pin la subió desde ~1.55 FALSEADA; el rodeo la bajó desde ~2.73; máx. 7). Adversario puro ~6.33. La SEVERIDAD es la métrica principal; la baja el coordinador posicionando drones para despejar pines activamente.)*
